...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            // nested-loops join.
//            try {
            int tableid = p.getTableId(j.t1Alias) != null?p.getTableId(j.t1Alias):
                    Database.getCatalog().getTableId(j.t1Alias);
            int singleBlock = BufferPool.getPageSize()/Database.getCatalog().getTupleDesc(tableid).getSize();
            return cost1 + Math.ceil((double)card1/(double)singleBlock) * cost2+ card1 * card2;
//            }catch (NullPointerException e){
//                return cost1 + 1 * cost2+ card1 * card2;
//            }
...
//        TableStats stats1 = stats.get(Database.getCatalog().getTableName(table1id));
//        TableStats stats2 = stats.get(Database.getCatalog().getTableName(table2id));
...
//        Vector<LogicalJoinNode> j = this.joins;
        CostCard card = null;
        PlanCache pc = new PlanCache();
        Set<LogicalJoinNode> joinNodeSet = null;
        double bestCost;
        int bestCard = 0;
        int count = 0;
        ...
        for (int i = 1; i <= joins.size(); i ++) {
            if (i == joins.size()){
                Set<LogicalJoinNode> s1 = new HashSet<>(joins);
                bestCost = Double.MAX_VALUE;
                bestPlan = new Vector<>(s1);
                for (LogicalJoinNode nodeToRemove : s1) {
                    card = computeCostAndCardOfSubplan(stats, filterSelectivities, nodeToRemove, s1, bestCost, pc);
                    if (card != null){
                        bestCost = card.cost;
                        bestPlan = card.plan;
                        bestCard = card.card;
                ...
                if (bestCost != Double.MAX_VALUE){
                    pc.addPlan(s1, bestCost, bestCard, new Vector<LogicalJoinNode> (bestPlan));
                }
                joinNodeSet = s1;
                break;
            }
            for (Set<LogicalJoinNode> s1 :enumerateSubsets(joins, i)){
                bestCost = Double.MAX_VALUE;
                bestPlan = new Vector<>(s1);
                for (LogicalJoinNode nodeToRemove : s1) {
                    count++;
        ...
        System.out.println(new Date()+"order join end");
        return pc.getOrder(joinNodeSet);
//        if (explain){
//            printJoins(bestPlan,);
//        }
//            for s in enumerateSubsets(this.joins, i):
//                bestPlan = {}
