...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		if (pid.pgcateg() == BTreePageId.LEAF){
			...
			BTreeInternalPage btintpage = (BTreeInternalPage) btreepage;
			Iterator<BTreeEntry> btiter = btintpage.iterator();
			if (f == null){
				return findLeafPage(tid,dirtypages,btiter.next().getLeftChild(),perm,f);
			}
			BTreeEntry entry = null;
			for (int i = 0; i < btintpage.getNumEntries(); i++) {
				entry = btiter.next();
				if (f.compare(LESS_THAN_OR_EQ, entry.getKey())){
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		BTreeLeafPage newLeafPage = (BTreeLeafPage) getEmptyPage(tid,dirtypages,2);


		// set sibling pointers.

		// split evenly
		Iterator<Tuple> moveTuple  =  page.reverseIterator();
		int moveNum = page.getNumTuples()/2;
		Tuple move = null;
		for (int i = 0; i < moveNum; i++) {
			move = moveTuple.next();
			page.deleteTuple(move);
			newLeafPage.insertTuple(move);
		}

		// copy up middle key
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid,dirtypages,page.getParentId(),move.getField(keyField));
		BTreeEntry newEntry =new BTreeEntry(move.getField(keyField),page.getId(),newLeafPage.getId());
		...
		if (page.getRightSiblingId()!=null){
			...
			newRightSibling.setLeftSiblingId(newLeafPage.getId());
		}
		...
		// return page
		if (field.compare(Op.GREATER_THAN_OR_EQ, move.getField(keyField))){
			return newLeafPage;
		}else{return page;}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
			BTreeInternalPage page, Field field)
					throws DbException, IOException, TransactionAbortedException {
		// some code goes here
		BTreeInternalPage newInternalPage = (BTreeInternalPage) getEmptyPage(tid,dirtypages,1);

		// split evenly
		Iterator<BTreeEntry> moveEntry  =  page.reverseIterator();
		int moveNum = page.getNumEntries()/2;
		BTreeEntry move = null;
		for (int i = 0; i < moveNum; i++) {
			move = moveEntry.next();
			page.deleteKeyAndRightChild(move);
			newInternalPage.insertEntry(move);
		}

		// push up middel key
		move = moveEntry.next();
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid,dirtypages,page.getParentId(),move.getKey());;
		BTreeEntry newEntry = new BTreeEntry(move.getKey(),page.getId(),newInternalPage.getId());;
		page.deleteKeyAndRightChild(move);
		parentPage.insertEntry(newEntry);
		updateParentPointers(tid,dirtypages,newInternalPage);

		// return page
		if (field.compare(GREATER_THAN_OR_EQ, move.getKey())){
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		Iterator<Tuple> steal;
		Tuple stealed = null;
		int needMoveNum = (page.getNumTuples() + sibling.getNumTuples() + 1)/2 - page.getNumTuples();
		if (isRightSibling){
			steal = sibling.iterator();
			for (int i = 0; i < needMoveNum; i++) {
				stealed = steal.next();
				sibling.deleteTuple(stealed);
				page.insertTuple(stealed);
			}
			stealed = steal.next();
			entry.setKey(stealed.getField(keyField));
			parent.updateEntry(entry);
		}else {
			steal = sibling.reverseIterator();
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// some code goes here
		Iterator<BTreeEntry> steal = leftSibling.reverseIterator();
		BTreePageId pageLeftChild = page.iterator().next().getLeftChild();
		BTreeEntry stealed;
		BTreeEntry toPage = null;
		int needMoveNum = (page.getNumEntries() + leftSibling.getNumEntries() + 1)/2 - page.getNumEntries();
		for (int i = 0; i < needMoveNum; i++) {
			stealed = steal.next();
			toPage = new BTreeEntry(parentEntry.getKey(),stealed.getRightChild(),pageLeftChild);
			pageLeftChild = stealed.getRightChild();
			updateParentPointer(tid,dirtypages,page.getId(),pageLeftChild);
			leftSibling.deleteKeyAndRightChild(stealed);
			page.insertEntry(toPage);
			parentEntry.setKey(stealed.getKey());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		Iterator<BTreeEntry> steal = rightSibling.iterator();
		BTreePageId pageRightChild = page.reverseIterator().next().getRightChild();
		BTreeEntry stealed;
		BTreeEntry toPage = null;
		int needMoveNum = (page.getNumEntries() + rightSibling.getNumEntries() + 1)/2 - page.getNumEntries();
		for (int i = 0; i < needMoveNum; i++) {
			stealed = steal.next();
			toPage = new BTreeEntry(parentEntry.getKey(),pageRightChild,stealed.getLeftChild());
			pageRightChild = stealed.getLeftChild();
			updateParentPointer(tid,dirtypages,page.getId(),pageRightChild);
			rightSibling.deleteKeyAndLeftChild(stealed);
			page.insertEntry(toPage);
			parentEntry.setKey(stealed.getKey());
			parent.updateEntry(parentEntry);
		}
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		Iterator<Tuple> merge = rightPage.iterator();
		Tuple move = null;
		while (merge.hasNext()){
			move = merge.next();
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		BTreeEntry pullParent = new BTreeEntry(parentEntry.getKey(),leftChild,rightChild);
		leftPage.insertEntry(pullParent);
		Iterator<BTreeEntry> merges = rightPage.iterator();
		BTreeEntry move = null;
		while (merges.hasNext()){
			move = merges.next();
			rightPage.deleteKeyAndLeftChild(move);
			leftPage.insertEntry(move);
			updateParentPointer(tid,dirtypages,leftPage.getId(), move.getLeftChild());
		}
		updateParentPointer(tid,dirtypages, leftPage.getId(), move.getRightChild());
		setEmptyPage(tid,dirtypages,rightPage.getId().pageNumber());
		deleteParentEntry(tid,dirtypages,leftPage,parent,parentEntry);
	}
	...
	NoSuchElementException {
		while (it != null) {
			...
			while (it.hasNext()) {
				...
				else if(ipred.getOp() == Op.LESS_THAN || ipred.getOp() == LESS_THAN_OR_EQ) {
